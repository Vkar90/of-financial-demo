export default class CurrencyInputLibrary {
    ...
    format(event) {
        ...
        let sepBef = 0;
        let endsWithDecimalBreak;
        if (typeof event !== 'undefined') {
            input = this.getEventValue(event).replaceAll(sap.symbol, '');
             sepBef = (separationChar !== '') ? input.length : 0;
            ...
            const decimalRegex = new RegExp(`^[^\\.]*\\.(\\d{0,${this.getCurrencyDecimalCount()}})?$`)
             endsWithDecimalBreak = input.match(decimalRegex) !== null;
        ...
        const sepShift = (Number.isNaN(sepAft - sepBef)) ? 0 : sepAft - sepBef;
    ...
    reinit(options) {
        ...
        this.disableCents = !!((typeof options.disableCents === 'boolean' && options.disableCents === true));
        this.preventInputFromIME = !((typeof options.preventInputFromIME !== 'undefined' && typeof options.preventInputFromIME === 'boolean' && options.preventInputFromIME === false));
    ...
    getCurrencySymbolAndPosition() {
        ...
        if (this.currencyDisplay === 'symbol') {
             symbol = (0).toLocaleString(this.locale, {
                style: 'currency',
    ...
    validateInput(e) {
        ...
        if (input === '-') {
             checkValue = 0;
        } else if (input.slice(-1) === decimalChar) {
                 checkValue = parseFloat(input.replace(new RegExp(`\\${decimalChar}`), '.') + (10**-decimals).toString().replace('0.', ''));
            } else {
                 checkValue = parseFloat(input.replace(new RegExp(`\\${decimalChar}`), '.'));
            }
        if (input.match(new RegExp(regex)) === null ||
        (this.min !== false && checkValue < this.min) ||
        (this.max !== false && checkValue > this.max) ||
        checkValue > 999999999999999 / 10**(this.disableCents ? 0 : decimals) ||
        checkValue < -999999999999999 / 10**(this.disableCents ? 0 : decimals) ||
